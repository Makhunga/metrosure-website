  ...
  return (
    <>
      <motion.header
        ...
        className={`fixed w-full z-50 transition-all duration-500 ${isDevBannerDismissed ? "top-0" : "top-10"
          } ${isScrolled
              ...
              {navLinks.map((link, index) => (
                ...
                >
                  {link.dropdown ? (
                    // Dropdown item
                    <button
                      className={`text-sm font-semibold transition-colors flex items-center gap-1 ${(link.label === "Insurance" && pathname.startsWith("/insurance")) ||
                    ...
                    <Link
                      href={link.href || "/"}
                      className={`text-sm font-semibold transition-colors relative flex items-center gap-1.5 ${isActive(link.href || "/")
                        ? "text-primary after:absolute after:bottom-[-30px] after:left-0 after:w-full after:h-0.5 after:bg-primary"
                        : "text-[rgb(var(--color-text-body))] hover:text-primary after:absolute after:bottom-[-30px] after:left-0 after:w-0 after:h-0.5 after:bg-primary after:transition-[width] after:duration-300 after:ease-out hover:after:w-full"
                        }`}
                    >
                      {link.label}
                      {link.badge && (
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded-full bg-green-500/10 text-green-600 dark:text-green-400 text-[10px] font-bold uppercase tracking-wider">
                          {link.badge}
                        </span>
                      )}
                    </Link>
                ...
                {navLinks.map((link, index) => (
                  ...
                  >
                    {link.dropdown ? (
                      ...
                      <div>
                        <button
                          className={`w-full text-base font-semibold transition-colors py-3 px-4 rounded-lg hover:bg-primary/5 flex items-center justify-between ${(link.label === "Insurance" && pathname.startsWith("/insurance")) ||
                            (link.label === "Tools" && pathname.startsWith("/tools"))
                      ...
                      <Link
                        href={link.href || "/"}
                        className={`text-base font-semibold transition-colors py-3 px-4 rounded-lg hover:bg-primary/5 flex items-center gap-2 ${isActive(link.href || "/")
                          ? "text-primary bg-primary/5"
                          : "text-[rgb(var(--color-text-body))] hover:text-primary"
                          }`}
