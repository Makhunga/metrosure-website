...
export default function PartnerInquiryForm() {
  ...
  const validateStepFields = (step: number): boolean => {
    if (step === 0) {
      const companyValid = validateField("companyName", formData.companyName, (v) => validateRequired(v, "Company name"));
      const businessTypeValid = validateField("businessType", formData.businessType, (v) => validateRequired(v, "Business type"));
      const locationsValid = validateField("numberOfLocations", formData.numberOfLocations, (v) => validateRequired(v, "Number of locations"));
      return companyValid && businessTypeValid && locationsValid;
    }
    if (step === 1) {
                    ...
                    {currentStep === 0 && (
                      ...
                      >
                        ...
                        <LabelledSelect
                          ...
                          required
                          onChange={handleInputChange}
                          fieldState={getFieldState("businessType")}
                          onBlur={() => validateField("businessType", formData.businessType, (v) => validateRequired(v, "Business type"))}
                          inputClassName={!formData.businessType ? "text-slate-400 dark:text-slate-500" : ""}
                        />
                        ...
                        <LabelledSelect
                          ...
                          required
                          onChange={handleInputChange}
                          fieldState={getFieldState("numberOfLocations")}
                          onBlur={() => validateField("numberOfLocations", formData.numberOfLocations, (v) => validateRequired(v, "Number of locations"))}
                          inputClassName={!formData.numberOfLocations ? "text-slate-400 dark:text-slate-500" : ""}
                        />
                    ...
                    {currentStep === 2 && (
                      ...
                      >
                        ...
                        <LabelledSelect
                          ...
                          onChange={handleInputChange}
                          inputClassName={!formData.currentFootTraffic ? "text-slate-400 dark:text-slate-500" : ""}
                        />
                  ...
                  <div className="flex items-center justify-between mt-10 pt-8 border-t border-slate-100 dark:border-slate-700">
                    ...
                    {currentStep < steps.length - 1 ? (
                      ...
                      <motion.button
                        ...
                        className="flex items-center gap-2 h-12 px-8 bg-primary hover:bg-[rgb(var(--color-primary-hover))] text-white font-bold rounded-xl shadow-lg shadow-primary/25 hover:shadow-primary/40 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                        whileHover={!isSubmitting && canProceed ? { scale: 1.03, y: -2 } : {}}
